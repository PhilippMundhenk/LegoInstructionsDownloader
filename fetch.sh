...
mkdir -p $2/$ID
cd $2/$ID
data="$(curl -s -X POST -H "Accept: application/json, text/plain, */*" -H "Accept-Language: de,en-US;q=0.7,en;q=0.3" -H "Accept-Encoding: gzip, deflate, br, zstd" -H "Content-Type: application/json" -H "x-api-key: $key" -H "Origin: https://www.lego.com" -H "DNT: 1" -H "Sec-GPC: 1" -H "Connection: keep-alive" -H "Referer: https://www.lego.com/" -H "Sec-Fetch-Dest: empty" -H "Sec-Fetch-Mode: cors" -H "Sec-Fetch-Site: same-site" https://services.slingshot.lego.com/api/v4/lego_historic_product_read/_search -d "{\"_source\":[\"product_number\",\"locale.$locale\",\"locale.en-us\",\"market.de.skus.item_id\",\"market.us.skus.item_id\",\"availability\",\"themes\",\"product_versions\",\"assets\"],\"from\":0,\"size\":1,\"query\":{\"bool\":{\"must\":[{\"term\":{\"product_number\":\"$ID\"}}],\"should\":[],\"filter\":[]}}}" | gunzip | python -m json.tool)"
echo "$data" > data.json
